...
import { getPackageAssets } from "@/actions/(protected)/business/assets/packages/getPackageAssets";
import { uploadPackageAssets } from "@/actions/(protected)/business/assets/packages/uploadPackageAssets";
import { uploadBusinessAssets } from "@/actions/(protected)/business/assets/uploadBusinessAssets";
import {
  getBusinessAssetsType,
  getPackageAssetsType,
    ...
    if (e.target.files) {
      ...
      if ([...allowedImageType, ...allowedVideoType].includes(files[0].type)) {
        ...
        if (allowedImageType.includes(files[0].type)) {
          ...
          res = await uploadPackageAssets({
            file: assetsForm,
            packageId: params.slug as string,
          });
        } else {
