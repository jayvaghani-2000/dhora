...
const SIGNER_COLOR_CLASS_PREFIX = "signer";

const getSignerColorClasses = (colorKey: string) => {
  const baseClass = `${SIGNER_COLOR_CLASS_PREFIX}-${colorKey}`;
  return {
    background: `${baseClass}-background`,
    base: `${baseClass}-base`,
    fieldItem: `${baseClass}-field-item`,
    fieldItemInitials: `${baseClass}-field-item-initials`,
    comboboxItem: `${baseClass}-combobox-item`,
  };
};

export const useSignerColors = (index: number) => {
  const key = AVAILABLE_SIGNER_COLORS[index % AVAILABLE_SIGNER_COLORS.length];
  return getSignerColorClasses(key);
};
