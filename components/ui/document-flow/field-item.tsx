...
import type { TFieldMetaSchema } from "@/lib/types/field-meta";
import { ZCheckboxFieldMeta, ZRadioFieldMeta } from "@/lib/types/field-meta";
...
export const FieldItem = ({
  ...
  const handleClickOutsideField = (event: MouseEvent) => {
    if (
      settingsActive &&
      $el.current &&
      !event.composedPath().includes($el.current)
    ) {
  ...
  return createPortal(
    ...
    >
      <div
        className={cn(
          "relative flex h-full w-full items-center justify-center rounded-lg bg-white",
          signerStyles.base,
