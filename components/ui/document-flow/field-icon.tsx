...
import type { TFieldMetaSchema as FieldMetaType } from "@/lib/types/field-meta";
...
export const FieldIcon = ({
  ...
  if (type === "SIGNATURE" || type === "FREE_SIGNATURE") {
    return (
      <div
        className={cn(
          "text-black flex items-center justify-center gap-x-1 text-xl",
          fontCaveatClassName
        )}
      >
        Signature
    ...
    if (fieldMeta && (type === "TEXT" || type === "NUMBER")) {
      if (
        ...
        label =
          fieldMeta.text.length > 10
            ? fieldMeta.text.substring(0, 10) + "..."
            : fieldMeta.text;
      } else if (fieldMeta.label) {
        label =
          fieldMeta.label.length > 10
            ? fieldMeta.label.substring(0, 10) + "..."
            : fieldMeta.label;
      } else {
    ...
    return (
      <div className="text-primary-black flex items-center justify-center gap-x-1.5 text-sm">
