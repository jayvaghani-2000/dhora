...
import { useDocumentElement } from "@/lib/client-only/hooks/use-document-element";
import { PDF_VIEWER_PAGE_SELECTOR } from "@/lib/constants/pdf-viewer";
import { RECIPIENT_ROLES_DESCRIPTION } from "@/lib/constants/recipient-roles";
import {
  type TFieldMetaSchema as FieldMeta,
  ZFieldMetaSchema,
} from "@/lib/types/field-meta";
import { RecipientRole } from "@/lib/types/recipient-role";
import { FieldFormType, FieldType } from "@/lib/types/field-type";
...
import { FieldItem } from "../document-flow/field-item";
import type { DocumentFlowStep } from "../document-flow/types";
import { FRIENDLY_FIELD_TYPE } from "../document-flow/types";
...
export const AddTemplateFieldsFormPartial = ({
  ...
  const {
    ...
    defaultValues: {
      fields: fields.map(field => ({
        nativeId: field.id,
        formId: `${field.id}-${field.template_id}`,
        pageNumber: field.pageNumber,
        type: field.type,
        pageX: Number(field.pageX),
        pageY: Number(field.pageY),
  ...
  const onFormSubmit = handleSubmit(onSubmit, error => {
    console.log(error);
  });
  ...
  const [selectedSigner, setSelectedSigner] = useState<any | null>(null);
  ...
  const onMouseClick = useCallback(
    (event: MouseEvent) => {
      ...
      append({
        ...
        recipient_id: selectedSigner.id,
  ...
  const recipientsByRoleToDisplay = useMemo(() => {
    return (
      Object.entries(recipientsByRole) as [RecipientRole, any[]][]
    ).filter(
            ...
            <div className="flex flex-col">
              ...
              {localFields.map((field, index) => {
                ...
                return (
                  <FieldItem
                    ...
                    onResize={(options: any) => onFieldResize(options, index)}
                    onMove={(options: any) => onFieldMove(options, index)}
              ...
              {!hideRecipients && (
                <Popover
                  ...
                  onOpenChange={setShowRecipientsSelector}
                >
