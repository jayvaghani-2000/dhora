...
import { ZRecipientAuthOptionsSchema } from "@/lib/types/document-auth";
import { nanoid } from "@/lib/universal/id";
import { generateRecipientPlaceholder } from "@/lib/utils/templates";
import { AnimateGenericFadeInOut } from "../animate/animate-generic-fade-in-out";
import { RecipientActionAuthSelect } from "../recipient/recipient-action-auth-select";
import { RecipientRoleSelect } from "../recipient/recipient-role-select";
...
export const AddTemplatePlaceholderRecipientsFormPartial = ({
  ...
  const generateDefaultFormSigners = () => {
    ...
    return recipients.map(recipient => ({
      nativeId: Number(recipient.id),
      formId: String(recipient.id),
      name: recipient.name,
      email: recipient.email,
      role: recipient.role,
      actionAuth:
        ZRecipientAuthOptionsSchema.parse(recipient.authOptions)?.actionAuth ??
        undefined,
    }));
  };
  ...
  const alwaysShowAdvancedSettings = useMemo(() => {
    const recipientHasAuthOptions = recipients.find(recipient => {
      const recipientAuthOptions = ZRecipientAuthOptionsSchema.parse(
        recipient.authOptions
      );

      return (
        recipientAuthOptions?.accessAuth || recipientAuthOptions?.actionAuth
      );
    });

    const formHasActionAuth = form
      .getValues("signers")
      .find(signer => signer.actionAuth);

    return (
      recipientHasAuthOptions !== undefined || formHasActionAuth !== undefined
    );
  }, [recipients, form]);
  ...
  const isSignerDirectRecipient = (
    ...
    if (!templateDirectLink) return false;
    return (
    ...
    <>
      ...
      <DocumentFlowFormContainerContent>
        ...
        <AnimateGenericFadeInOut
          motionKey={showAdvancedSettings ? "Show" : "Hide"}
        >
          <Form {...form}>
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="text-foreground z-9999 max-w-md p-4">
                          ...
                          <p className="text-muted-foreground mt-1">
                            This field cannot be modified or deleted. When you
                            share this template's direct link or add it to your
                            public profile, anyone who accesses it can input
                            their name and email, and fill in the fields
                            assigned to them.
                          </p>
                        </TooltipContent>
                      </Tooltip>
            ...
            >
              ...
              <Button
                ...
                disabled={
                  ...
                  form
                    .getValues("signers")
                    .some(signer => signer.email === user?.email)
                }
