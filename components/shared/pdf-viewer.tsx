...
export const PDFViewer = ({
  ...
  const passwordCallbackRef = useRef<
  ...
  useEffect(() => {
    async function fetchDocumentBytes() {
      if (documentData?.length) {
        const bytes = base64?.decode(documentData);
        ...
        <>
          <PDFDocument
            ...
            onPassword={(callback, reason) => {
              ...
              match(reason)
                .with(PasswordResponses.NEED_PASSWORD, () =>
                  setIsPasswordError(false)
                )
                .with(PasswordResponses.INCORRECT_PASSWORD, () =>
                  setIsPasswordError(true)
                );
            }}
            onLoadSuccess={d => onDocumentLoaded(d as any)}
