...
const getMyBusinessDetailHandler = async (user: User, businessId?: string) => {
...
const handler = async (user: User, businessId?: string) => {
  try {
    const businessDetail = await getMyBusinessDetailHandler(user, businessId);
    ...
    return {
      ...errorHandler(err),
      data: {} as NonNullable<
        Awaited<ReturnType<typeof getMyBusinessDetailHandler>>
      >,
    };
  }
};

export const getMyBusinessDetails: (
  businessId?: string
